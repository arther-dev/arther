...
| Migrations (12) | [`supabase/migrations/`](supabase/migrations/) (canonical) ⇄ [`Development/Architecture/migrations/`](Development/Architecture/migrations/) (reference) | Phase-ordered executable schema (0012 added during F5: atomic field-value RPC) |
...
| 2026-06-11 | M1 verification + turbo env fix | Verified manual provisioning: Vercel `arther-app`+`arther-portal` live with env vars (F0.3 done), `SENTRY_AUTH_TOKEN`/`SUPABASE_SERVICE_ROLE_KEY` set; F0.4 acceptance fired — `/sentry-check?go=1` reached Sentry (ARTHER-APP-1/2) but frames unmapped. Root-caused from Vercel build logs: turbo strict env stripped undeclared `SENTRY_*`/`SUPABASE_*` from `app#build`; pnpm blocked `@sentry/cli` postinstall. Fix: `turbo.json` build `env`+`passThroughEnv`, `@sentry/cli` in `onlyBuiltDependencies`; PROVISIONING.md status updated | M1 tick still pending: redeploy → re-run `/sentry-check?go=1` → confirm source-mapped frames. Auth settings (step 3) unverifiable via connector — dashboard check. `SENTRY_AUTH_TOKEN` Production-only (add Preview). PITR still blocked on free plan |
