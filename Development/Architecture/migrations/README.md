...
| `0020_approval_override.sql` | C1.5: `override_approval()` — the document owner (or a workspace admin) approves on behalf of a missing reviewer with a mandatory reason, recording an `owner_override` (carrying the overridden role label) + a flagged `audit_log` entry, then re-evaluating the AND-logic gate. SECURITY DEFINER — self-authorizes owner/admin and writes the deny-all audit row + the editor-gated transition. |
| `0021_publish_document.sql` | C4.2–C4.5: `publish_document()` — atomically freezes an **approved** revision into a versioned, immutable `published_snapshots` row (resolved `block_tree` + `resolution_manifest` + `search_text`, `pdf_ready=false`) and flips the revision to Published. Monotonic semantic version per document (1.0, 2.0, …); the 0008 freeze guard makes the content immutable thereafter. Service-role only (the publish pipeline writes snapshots; the app authorizes `doc.publish` first), attribution passed in. |
