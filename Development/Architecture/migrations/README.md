...
| `0017_document_type_fork.sql` | G0.1 Document Types: `fork_document_type()` — atomically copies a built-in Document Type plus its sections and approval roles into the caller's workspace as an editable copy (`built_in=false`, `forked_from` set); the built-in stays canonical. Invoker rights, so admin RLS (document_types_write) governs the inserts. Type/section/role tables ship in 0004. |
