...
| `0017_document_type_fork.sql` | G0.1 Document Types: `fork_document_type()` — atomically clones a Document Type (type + sections + approval roles) into an editable workspace copy with `forked_from` lineage; invoker rights, admin RLS governs (built-ins are forkable-not-editable, spec §3.4). |
